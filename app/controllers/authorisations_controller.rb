class AuthorisationsController < ApplicationController
  ...
  def show
    ...
    @threads = @authorisation.email_threads.joins(:tags).where(synced: true).filter(params_filters).includes(:email_messages, :message_attachments, :message_participants, :participants).all.paginate(page: params[:page], :per_page => 10)
